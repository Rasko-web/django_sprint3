...
def set_published_post(request):
    return (
        request.
        filter(
            is_published=True,
            category__is_published=True,
            pub_date__date__lte=datetime.datetime.today()
            # я не понимаю тогда, как нужно исправить
        ))
...
def index(request):
    ...
    context = {
        'post_list': set_published_post(post_list,)[:post_count]
    }
